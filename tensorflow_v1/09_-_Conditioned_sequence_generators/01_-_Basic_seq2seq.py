...
with g.as_default():
    ...
    with tf.variable_scope('sources'):
        ...
        (_, (state_fw_sources, state_bw_sources)) = tf.nn.bidirectional_dynamic_rnn(cell_fw_sources, cell_bw_sources, embedded_sources, sequence_length=source_lens, initial_state_fw=batch_init_fw_sources, initial_state_bw=batch_init_bw_sources)
        W = tf.get_variable('W', [ 2*state_size, state_size ], tf.float32, tf.random_normal_initializer(stddev=1.0, seed=0))
        state_sources = tf.matmul(tf.concat([state_fw_sources, state_bw_sources], axis=1), W)
    ...
    with tf.variable_scope('prefixes'):
        ...
        batch_init_prefixes = state_sources
    ...
    with tf.Session() as s:
        ...
        [ curr_probs ] = s.run([ probs ], { sources: padded_indexes, source_lens: index_lens, prefixes: padded_index_prefixes, prefix_lens: index_prefix_lens })
