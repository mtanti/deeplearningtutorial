...
with g.as_default():
    ...
    with tf.variable_scope('embedding'):
        ...
        embedded_ = tf.nn.embedding_lookup(embedding_matrix, prefixes)
        
        #Attach conditioning vector to front of embedded words (resized so that it has the same size as the embedded word vectors)
        
        W = tf.get_variable('W', [conditionings_size, embedding_size], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
        b = tf.get_variable('b', [embedding_size], tf.float32, tf.zeros_initializer())
        conditionings_ = tf.reshape(tf.matmul(conditionings, W) + b, [batch_size, 1, conditionings_size])
        
        embedded = tf.concat([ conditionings_, embedded_ ], axis=1)
    ...
    with tf.variable_scope('rnn'):
        ...
        (outputs_, _) = tf.nn.dynamic_rnn(cell, embedded, sequence_length=prefix_lens+1, initial_state=batch_init) #Add 1 to prefix lens to include conditioning vector
        outputs = outputs_[:, 1:, :] #Drop the output resulting from the conditioning vector
