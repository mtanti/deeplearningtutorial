...
with g.as_default():
    ...
    with tf.variable_scope('rnn'):
        ...
        (outputs_, _) = tf.nn.dynamic_rnn(cell, embedded, sequence_length=prefix_lens, initial_state=batch_init)
        
        #Attach conditioning vector to each resulting RNN state (no need to resize conditioning vector)
        
        conditionings_ = tf.tile(tf.reshape(conditionings, [batch_size, 1, conditionings_size]), [1, seq_width, 1])
        outputs = tf.concat([ conditionings_, outputs_ ], axis=2)

    with tf.variable_scope('output'):
        W = tf.get_variable('W', [state_size + conditionings_size, len(vocab)], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
        b = tf.get_variable('b', [len(vocab)], tf.float32, tf.zeros_initializer())
        outputs_2d = tf.reshape(outputs, [batch_size*seq_width, state_size + conditionings_size])
