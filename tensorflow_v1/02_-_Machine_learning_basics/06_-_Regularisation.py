...
class Model(object):
    ...
    def __init__(self, degree):
        ...
        weight_decay_weight = 0.1 #This is the amount of importance to give to minimising the weight magnitudes
        ...
        with self.graph.as_default():
            ...
            params_size = tf.constant(0.0, tf.float32, [])
            for coeff in self.coeffs[1:]:
                params_size = params_size + coeff**2 #Find the magnitude of all parameters except the constant term.
            loss = self.error + weight_decay_weight*params_size

            self.optimiser_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss)
