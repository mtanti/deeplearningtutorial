...
with g.as_default():
    ...
    with tf.variable_scope('source'):
        ...
        #Encode source sentence word vectors using bi-directional RNN
        with tf.variable_scope('rnn'):
            ...
            outputs_2d = tf.matmul(outputs_2d_, W) + b
            outputs = tf.reshape(outputs_2d, [batch_size, source_seq_width, source_state_size])
        
        #Produce an attention vector based on each source sentence word vector and take the weighted average of each word vector
        with tf.variable_scope('attention'):
            W1 = tf.get_variable('W1', [source_state_size, preattention_size], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
            b1 = tf.get_variable('b1', [preattention_size], tf.float32, tf.zeros_initializer())
            pre_attention_2d = tf.tanh(tf.matmul(outputs_2d, W1) + b1)
            ...
            attention_logits = tf.reshape(tf.matmul(pre_attention_2d, W2) + b2, [batch_size, source_seq_width])

            mask = tf.sequence_mask(source_lens, source_seq_width, tf.float32)
            attention = tf.nn.softmax(attention_logits*mask + -1e10*(1 - mask)) #The attention vector (with pad values being set to zeros)

            expanded_attention = tf.tile(tf.reshape(attention, [batch_size, source_seq_width, 1]), [1, 1, source_state_size]) #Make attention vector same shape as word vectors
            attended_sources = tf.reduce_sum(outputs*expanded_attention, axis=1) #Weighted average
        
        source_encoding = attended_sources
    ...
    with tf.Session() as s:
        ...
        for sent in source_tokens:
            ...
            [ curr_attention ] = s.run([ attention ], { source_indexes: [ source ], source_lens: [ len(source) ] })
            
            print('Attention:         ', np.round(curr_attention[0, :], 2))
