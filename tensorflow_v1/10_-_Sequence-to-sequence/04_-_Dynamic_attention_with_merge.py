...
with g.as_default():
    ...
    with tf.variable_scope('source'):
        ...
        with tf.variable_scope('rnn'):
            ...
            source_outputs = tf.reshape(tf.matmul(outputs_2d_, W) + b, [batch_size, source_seq_width, source_state_size])
    ...
    with tf.variable_scope('targets'):
        ...
        with tf.variable_scope('rnn'):
            ...
            (target_outputs, _) = tf.nn.dynamic_rnn(cell, embedded, sequence_length=target_lens, initial_state=batch_init)

        with tf.variable_scope('attention'):
            #Concatenate every source word vector to every language model state
            expanded_sources = tf.tile(tf.reshape(source_outputs, [batch_size, 1, source_seq_width, source_state_size]), [1, target_seq_width, 1, 1])
            expanded_targets = tf.tile(tf.reshape(target_outputs, [batch_size, target_seq_width, 1, target_state_size]), [1, 1, source_seq_width, 1])
            pre_attention_input = tf.concat([
                    tf.reshape(expanded_sources, [batch_size, target_seq_width*source_seq_width, source_state_size]),
                    tf.reshape(expanded_targets, [batch_size, source_seq_width*target_seq_width, target_state_size]),
                ], axis=2)
            pre_attention_input_2d = tf.reshape(pre_attention_input, [batch_size*source_seq_width*target_seq_width, source_state_size+target_state_size])
            
            W1 = tf.get_variable('W1', [source_state_size + target_state_size, preattention_size], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
            b1 = tf.get_variable('b1', [preattention_size], tf.float32, tf.zeros_initializer())
            pre_attention_2d = tf.tanh(tf.matmul(pre_attention_input_2d, W1) + b1)

            W2 = tf.get_variable('W2', [preattention_size, 1], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
            b2 = tf.get_variable('b2', [1], tf.float32, tf.zeros_initializer())
            attention_logits = tf.reshape(tf.matmul(pre_attention_2d, W2) + b2, [batch_size, target_seq_width, source_seq_width])

            mask = tf.tile(tf.reshape(tf.sequence_mask(source_lens, source_seq_width, tf.float32), [batch_size, 1, source_seq_width]), [1, target_seq_width, 1])
            attentions = tf.nn.softmax(attention_logits*mask + -1e10*(1 - mask))

            expanded_attention = tf.tile(tf.reshape(attentions, [batch_size, target_seq_width, source_seq_width, 1]), [1, 1, 1, source_state_size])
            attended_sources = tf.reduce_sum(expanded_sources*expanded_attention, axis=2)
        
        with tf.variable_scope('output'):
            W = tf.get_variable('W', [target_state_size + source_state_size, len(target_vocab)], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
            b = tf.get_variable('b', [len(target_vocab)], tf.float32, tf.zeros_initializer())
            output_inputs = tf.concat([ target_outputs, attended_sources ], axis=2) #Concatenate attended vector to language model state before passing them to softmax
            output_inputs_2d = tf.reshape(output_inputs, [batch_size*target_seq_width, target_state_size + source_state_size])
            logits_2d = tf.matmul(output_inputs_2d, W) + b
