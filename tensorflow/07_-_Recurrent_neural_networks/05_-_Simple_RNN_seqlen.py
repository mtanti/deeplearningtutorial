...
prefix_lens = [ sum(token != '<PAD>' for token in prefix) for prefix in token_prefixes ]
...
with g.as_default():
    ...
    cell = Cell()
    (outputs, state) = tf.nn.dynamic_rnn(cell, embedded, sequence_length=sent_lens, initial_state=batch_init)
    ...
    step = tf.train.GradientDescentOptimizer(0.5).minimize(error)
    ...
    with tf.Session() as s:
        ...
        for epoch in range(1, 5000+1):
            ...
            if epoch%100 == 0:
                ...
                ax[1].cla()
                for (token_prefix, prefix_len, index_prefix) in zip(token_prefixes, prefix_lens, index_prefixes):
                    [ curr_prefix_vec ] = s.run([ state ], { sents: [index_prefix], sent_lens: [prefix_len] })
        ...
        print()
        for (sent, sent_len, index) in zip(tokens, token_lens, indexes):
            [ curr_prefixes_vec ] = s.run([ outputs ], { sents: [index], sent_lens: [sent_len] })
