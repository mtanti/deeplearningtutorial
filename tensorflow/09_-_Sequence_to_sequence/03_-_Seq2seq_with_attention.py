...
with g.as_default():
    ...
    with tf.variable_scope('prefixes'):
        ...
        class CellAttention(tf.nn.rnn_cell.RNNCell):
            ...
            @property
            def output_size(self):
                return (num_steps_sources, state_size)

            def build(self, inputs_shape):
                self.W1 = self.add_variable('W1', [ state_size+state_size, attention_size ], tf.float32, tf.random_normal_initializer(stddev=0.01, seed=0))
                self.W2 = self.add_variable('W2', [ attention_size, 1 ], tf.float32, tf.random_normal_initializer(stddev=0.01, seed=0))
                self.W3 = self.add_variable('W3', [ state_size+state_size+embedding_size, state_size ], tf.float32, tf.random_normal_initializer(stddev=1.0, seed=0))
                self.b3 = self.add_variable('b3', [ state_size ], tf.float32, tf.zeros_initializer())
                self.built = True
                
            def call(self, x, curr_state):
                with tf.variable_scope('attention'):
                    expanded_curr_state = tf.tile(tf.reshape(curr_state, [ batch_size, 1, state_size ]), [ 1, num_steps_sources, 1 ])
                    expanded_curr_state_2d = tf.reshape(expanded_curr_state, [ batch_size*num_steps_sources, state_size ])
                    attention_input_2d = tf.concat([ outputs_sources_2d, expanded_curr_state_2d ], axis=1)
                    pre_attention_2d = tf.tanh(tf.matmul(attention_input_2d, self.W1))

                    attention_logits_2d = tf.matmul(pre_attention_2d, self.W2)
                    ...
                    attended_source = tf.reduce_sum(outputs_sources*expanded_attention, axis=1)
                    
                state_input = tf.concat([ attended_source, curr_state, x ], axis=1)
                new_state = tf.tanh(tf.matmul(state_input, self.W3) + self.b3)
                return ((attention, new_state), new_state)

        
        cell_prefixes = CellAttention()
        ((attention, outputs_prefixes), _) = tf.nn.dynamic_rnn(cell_prefixes, embedded_prefixes, sequence_length=prefix_lens, initial_state=batch_init_prefixes)
    ...
    with tf.Session() as s:
        ...
        for (sent, sent_probs, sent_attention) in zip(tokens, curr_probs.tolist(), curr_attention.tolist()):
            ...
            for i in range(len(sent)+1):
                ...
                token_attention = sent_attention[i]
                top_predictions = sorted(zip(token_probs, vocab), reverse=True)[:3]
                print(' '.join(sent))
                print(*[round(a, 2) for a in token_attention], sep=' ')
                print(' '.join(prefix))
